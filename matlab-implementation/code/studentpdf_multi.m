function p = studentpdf_multi(x, mu, covar, nu, dim)
...
% x: 1xd vector
% mu: nxd matrix 
% covar: n*dxd symmetric, positive definite matrix
% nu: positive scalar
...
for i = 1:m
    ...
    logc = gammaln(nu(i)/2 + dim/2) - gammaln(nu(i)/2) ...
        - 0.5*log(det(covar((2*i-1):2*i,:))) ...
        - (dim/2)*log(nu(i)) - (dim/2)*log(pi);
    p(i) = exp(logc)*(1 + exp(log1p(mult(i)/nu(i)))^(-(nu(i)+dim)/2));
end
